...
const feedUsageSchema = new mongoose.Schema({
  ...
  costAnalysis: {
    ...
    dailyCost: {
      type: Number,
      default: function() {
        const costPerKg = this.costAnalysis?.costPerKg || 0;
        return this.quantityUsed * costPerKg;
      }
    },
    costPerBird: {
      type: Number,
      default: function() {
        const dailyCost = this.costAnalysis?.dailyCost || 0;
        return this.totalBirds > 0 ? parseFloat((dailyCost / this.totalBirds).toFixed(2)) : 0;
      }
    }
  },
  notes: {
...
feedUsageSchema.pre('save', function(next) {
  ...
  if (this.costAnalysis?.costPerKg > 0) {
    ...
    if (this.totalBirds > 0) {
      this.costAnalysis.costPerBird = parseFloat((this.costAnalysis.dailyCost / this.totalBirds).toFixed(2));
    }
  }
