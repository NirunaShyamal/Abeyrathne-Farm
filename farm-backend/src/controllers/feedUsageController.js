...
const createFeedUsage = async (req, res) => {
  try {
    ...
    // Get active stock for this feed type (check both type and feedType fields)
    let stock = await FeedStock.findOne({ 
      feedType, 
      status: 'Active' 
    });
    
    // If not found by feedType, try by type field
    if (!stock) {
      stock = await FeedStock.findOne({ 
        type: feedType, 
        status: 'Active' 
      });
    }
    ...
    if (!stock) {
      return res.status(400).json({
        success: false,
        message: `No active stock found for ${feedType}. Please add stock first.`
      });
    }
    ...
    // Deduct from stock
    console.log(`📊 Before deduction - Stock: ${stock.feedType || stock.type}, Current: ${stock.currentQuantity} KG`);
    await stock.deductUsage(quantityUsed);
    console.log(`📊 After deduction - Stock: ${stock.feedType || stock.type}, Current: ${stock.currentQuantity} KG`);
